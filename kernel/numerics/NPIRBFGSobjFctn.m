...
elseif ~exist('yc','var') || isempty(yc),
  ...
  FAIRmessage('Non-Parametric Image Registration with BFGS','-'); 
...
yRef = y0;
fctn = @(yc) NPIRBFGSobjFctn(T,Rc,omega,m,yRef,yc); 
fctn([]);   % report status
...
OPTpara = FAIRcell2struct(optPara('lBFGS','maxIter',25));
[yOptc,his] = lBFGS(fctn,y0,OPTpara{:},'Plots',@FAIRplots); 
