...
grid = regularizer;
n = size(omega,2)/2*prod(m);
switch grid,
  case 'mbElastic', 
    if size(P,1) ~= n, P = stg2center(m);   end; % update
  ...
  case {'mbHyperElastic','mbElasticFEM'},
    if size(P,1) ~= n, P = nodal2center(m); end; % update
  case {'mfElasticNodal','mfHyperElastic','mfElasticFEM'},
