...
void volume3D( double *volu, const double *yc, const double *m,const int n){
    ...
    const double *rptr;
#pragma omp parallel  default(shared) private(rptr,id, i1, i2, i3, r, comp,idn, vol, yA, yB, yC, yD, yE, yF, yG, yH, yABDC, yBDHF, yABFE, yCAEG, yDCGH, yEFHG, yM)
    {
#pragma omp for
        for (i1=0; i1<m1; i1++){
            for (i2=0; i2<m2; i2++){
...
void S2D(double *S, double *dS, const double *yc,const double *VRef, const double *alpha ,const double *h,const double *m,const int n, const bool doDerivative){
    ...
    double *wptr;
    if (alphaVolume >0 ) {
#pragma omp parallel default(shared) private(wptr,ptr,id,idn, i1, i2, r, comp,sumvoxel, yA, yB, yC, yD, yM, rA, rB, rC, rD, rM) reduction(+: sum)
        {
#pragma omp for
            for (i1=0; i1<m1; i1++){
                for (i2=0; i2<m2; i2++){
...
#pragma mark -
#pragma mark EPI
void EPIdiagTriangle(double *y1, double *y2, double *yM, double *r1, double *r2, double *r3, double *r4, 
                         const double *v, double Ti){
    double dVx, cof[4];
    cofactor2D(cof, y1, y2, yM,1.0);
    ...
    // now we have 4 cases inside this triangle 
    //  1) x1[0] = v[0] --> xM[0] = 1/4*v[0] --> write to r1
    dVx = cof[0] * (0.75) + cof[1] * (-0.25);
    *r1 += Ti * dVx* v[0];
    
    //  2) x1[1] = v[1] --> xM[1] = 1/4 * v[0]--> write to r1
    dVx = cof[2] * (0.75) + cof[3] * (-0.25);
    *r1 += Ti * dVx * v[1];
    
    //  3) x2[0] = v[0] --> xM[0] = 1/4 * v[0]--> write to r2
    dVx = cof[0] * (-0.25) +  cof[1] * (0.75);
    *r2 += Ti * dVx *v[0];
    
    //  4) x2[1] = v[1] --> xM[1] = 1/4*v[1] --> write to r2
    dVx =  cof[2] * (-0.25) + cof[3] * (0.75) ;
    *r2 += Ti * dVx *v[1];
    
    
    // and we have 2 cases outside this triangle 
    //  1) x3[0] = v[0] or x4[0] =v[0]--> xM[0] = 1/4*v[0] --> write to r3 and r4
    dVx = (cof[0] + cof[1]) * (-0.25);
    *r3 += Ti * dVx *v[0];
    *r4 += Ti * dVx *v[0];
    
    //  2) x3[1] = v[1] or x4[1] --> xM[1] = 1/4*v[1]  --> write to r3 and r4
    dVx = (cof[2]  + cof[3]) * (-0.25);
    *r3 += Ti * dVx * v[1] ;
    *r4 += Ti * dVx * v[1];
    
}
void EPIdiag2D(double *diag, const double *yc1, const double *yc2, const double *m, const double *Jac1, 
               const double *Jac2, const double *dI1,const double *dI2, const double *I1, const double *I2, 
               const double *v1, const double *v2,const double *h){
    int idn, i1, i2, r, comp,id;
    double yc1A[2], yc1B[2], yc1C[2], yc1D[2], yc1M[2];
    double yc2A[2], yc2B[2], yc2C[2], yc2D[2], yc2M[2];
    double dI1i[2],Jac1i,I1i,dI2i[2],Jac2i,I2i;
    double rA, rB, rC, rD;
    ...
    const double *ptr;
    double *wptr;
#pragma omp parallel private(wptr,ptr,idn, i1, i2, r, comp,dI1i,Jac1i,I1i,dI2i,Jac2i,I2i,yc1A,yc1B,yc1C,yc1D,yc1M,yc2A,yc2B,yc2C,yc2D,yc2M,rA,rB,rC,rD)
    {
#pragma omp for
        for (i1=0; i1<m1; i1++){
            for (i2=0; i2<m2; i2++){
                ...
                for (r=0; r<2; r++) {
                    // get y at edges
                    ptr = yc1 + idn+comp;
                    yc1A[r] = *ptr; // yc[idn           +comp]; 
                    ptr = ptr+inc_x;
                    yc1B[r] = *ptr; //yc[idn+1         +comp]; 
                    ptr = ptr+inc_y;
                    yc1D[r] = *ptr; //yc[idn+1+(m1+1)  +comp]; 
                    ptr = ptr-inc_x;
                    yc1C[r] = *ptr; //* yc[idn  +(m1+1)  +comp]; 
                    yc1M[r] = 0.25*(yc1A[r]+yc1B[r]+yc1C[r]+yc1D[r]); 
            
                    ptr = yc2 + idn+comp;
                    yc2A[r] = *ptr; // yc[idn           +comp]; 
                    ptr = ptr+inc_x;
                    yc2B[r] = *ptr; //yc[idn+1         +comp]; 
                    ptr = ptr+inc_y;
                    yc2D[r] = *ptr; //yc[idn+1+(m1+1)  +comp]; 
                    ptr = ptr-inc_x;
                    yc2C[r] = *ptr; //* yc[idn  +(m1+1)  +comp]; 
                    yc2M[r] = 0.25*(yc2A[r]+yc2B[r]+yc2C[r]+yc2D[r]); 
                    
                    // init private result vectors (private)
                    rA += 0.25 * ( Jac1i * dI1i[r] * v1[r] - Jac2i * dI2i[r] * v2[r]) ;
                    comp+=nn;
                ...
                rD = rA;
                // do the work (private)
                EPIdiagTriangle(yc1A, yc1B, yc1M, &rA, &rB, &rD, &rC,v1,I1i);
                EPIdiagTriangle(yc1B, yc1D, yc1M, &rB, &rD, &rC, &rA,v1,I1i);
                EPIdiagTriangle(yc1D, yc1C, yc1M, &rD, &rC, &rA, &rB,v1,I1i);
                EPIdiagTriangle(yc1C, yc1A, yc1M, &rC, &rA, &rB, &rD,v1,I1i);                
    
                // do the work (private)
                EPIdiagTriangle(yc2A, yc2B, yc2M, &rA, &rB, &rD, &rC,v2,-I2i);
                EPIdiagTriangle(yc2B, yc2D, yc2M, &rB, &rD, &rC, &rA,v2,-I2i);
                EPIdiagTriangle(yc2D, yc2C, yc2M, &rD, &rC, &rA, &rB,v2,-I2i);
                EPIdiagTriangle(yc2C, yc2A, yc2M, &rC, &rA, &rB, &rD,v2,-I2i);
                // write into result vector (private --> shared)
                    wptr = diag + idn;
                    //dS[idn     +comp] += rA[r]+0.25*rM[r];
                    #pragma omp atomic
...
inline void EPIdiagTetra(double *y1, double *y2, double *yF, double *yM, 
                             double *r1, double *r2, double *r3, double *r4, 
                             double *r5, double *r6, double *r7, double *r8, const double *v,double Ti){
    double dVx,cof[9];
    
    cofactor3D(cof, y1, y2, yF, yM,1.0);
    
    // now there are 6 cases directly on this tetrahedra
    // 1) x1[0] = v[0] --> xF[0] = 0.25*v[0] and xM[0] = 0.125*v[0] ==> write to r1
    dVx = cof[0] * (1-0.125) + cof[1] * (-0.125) + cof[2] * (0.25-0.125);
    *r1 += Ti * dVx *v[0];
    // 2) x1[1] = 1 --> xF[1] = 0.25 and xM[1] = 0.125 ==> w*rite to r1[1]
    dVx = cof[3] * (1.0-0.125) + cof[4] * (-0.125) + cof[5] * (0.25-0.125);
    *r1 += Ti * dVx*v[1];
    // 3) x1[2] = 1 --> xF[2] = 0.25 and xM[2] = 0.125 ==> write to r1[2]
    dVx = cof[6] * (1.0-0.125) + cof[7] * (-0.125) + cof[8] * (0.25-0.125);
...
void EPIdiag3D(double *diag, const double *yc1, const double *yc2, const double *m, const double *Jac1, const double *Jac2, 
               const double *dI1,const double *dI2, const double *I1, const double *I2, const double *v1, const double *v2,const double *h){
    const int m1 = m[0], m2 = m[1],m3 = m[2];
    double yc1A[3], yc1B[3], yc1C[3], yc1D[3], yc1E[3], yc1F[3], yc1G[3], yc1H[3], yc1ABDC[3], yc1BDHF[3], yc1ABFE[3], yc1CAEG[3], yc1DCGH[3], yc1EFHG[3], yc1M[3];
    double yc2A[3], yc2B[3], yc2C[3], yc2D[3], yc2E[3], yc2F[3], yc2G[3], yc2H[3], yc2ABDC[3], yc2BDHF[3], yc2ABFE[3], yc2CAEG[3], yc2DCGH[3], yc2EFHG[3], yc2M[3];
    double rA, rB, rC, rD, rE, rF, rG, rH;
    
    double dI1i[3],dI2i[3],Jac1i,Jac2i,I1i,I2i;
    int id, i1, i2, i3, r, comp,idn;
    const double hd = h[0] * h[1] * h[2];
...
#pragma omp parallel for  default(shared) private(rptr,wptr,id, i1, i2, i3, r, comp,idn, dI1i,dI2i,Jac1i,Jac2i,I1i,I2i, yc1A, yc1B, yc1C, yc1D, yc1E, yc1F, yc1G, yc1H, yc1ABDC, yc1BDHF, yc1ABFE, yc1CAEG, yc1DCGH, yc1EFHG, yc1M, yc2A, yc2B, yc2C, yc2D, yc2E, yc2F, yc2G, yc2H, yc2ABDC, yc2BDHF, yc2ABFE, yc2CAEG, yc2DCGH, yc2EFHG, yc2M, rA, rB, rC, rD, rE, rF, rG, rH)
    for (i1=0; i1<m1; i1++){
        for (i2=0; i2<m2; i2++){
            for (i3=0; i3<m3; i3++){
                idn = i1+ i2* inc_y+ i3*inc_z;
                ...
                for(r=0;r<3;r++){
                    ...
                    yc1ABDC[r] = 0.25*(yc1A[r]+yc1B[r]+yc1D[r]+yc1C[r]);
                    yc1BDHF[r] = 0.25*(yc1B[r]+yc1D[r]+yc1H[r]+yc1F[r]);
                    yc1ABFE[r] = 0.25*(yc1A[r]+yc1B[r]+yc1F[r]+yc1E[r]);
                    yc1CAEG[r] = 0.25*(yc1C[r]+yc1A[r]+yc1E[r]+yc1G[r]);
                    yc1DCGH[r] = 0.25*(yc1D[r]+yc1C[r]+yc1G[r]+yc1H[r]);
                    yc1EFHG[r] = 0.25*(yc1E[r]+yc1F[r]+yc1H[r]+yc1G[r]);
                    yc1M[r]    = 0.5 *(yc1ABDC[r]+yc1EFHG[r]);
                    ...
                    yc2ABDC[r] = 0.25*(yc2A[r]+yc2B[r]+yc2D[r]+yc2C[r]);
                    yc2BDHF[r] = 0.25*(yc2B[r]+yc2D[r]+yc2H[r]+yc2F[r]);
                    yc2ABFE[r] = 0.25*(yc2A[r]+yc2B[r]+yc2F[r]+yc2E[r]);
                    yc2CAEG[r] = 0.25*(yc2C[r]+yc2A[r]+yc2E[r]+yc2G[r]);
                    yc2DCGH[r] = 0.25*(yc2D[r]+yc2C[r]+yc2G[r]+yc2H[r]);
                    yc2EFHG[r] = 0.25*(yc2E[r]+yc2F[r]+yc2H[r]+yc2G[r]);
                    yc2M[r]    = 0.5 *(yc2ABDC[r]+yc2EFHG[r]);
                    
                    comp +=nn;
                    
                    rA += 0.125 * (Jac1i*dI1i[r]*v1[r] - Jac2i*dI2i[r]*v2[r]);
                }
                rB = rA; rC = rA; rD = rA; rE = rA; rF = rA; rH = rA, rG = rA; rH = rA;
                
                EPIdiagTetra(yc1A, yc1B, yc1ABDC, yc1M, &rA, &rB, &rD, &rC, &rE, &rF, &rH, &rG,v1,-I1i);
                EPIdiagTetra(yc1B, yc1D, yc1ABDC, yc1M, &rB, &rD, &rC, &rA, &rF, &rH, &rG, &rE,v1,-I1i);
                EPIdiagTetra(yc1D, yc1C, yc1ABDC, yc1M, &rD, &rC, &rA, &rB, &rH, &rG, &rE, &rF,v1,-I1i);
                EPIdiagTetra(yc1C, yc1A, yc1ABDC, yc1M, &rC, &rA, &rB, &rD, &rG, &rE, &rF, &rH,v1,-I1i);
                
                EPIdiagTetra(yc1B, yc1D, yc1BDHF, yc1M, &rB, &rD, &rH, &rF, &rA, &rC, &rG, &rE,v1,I1i);
                EPIdiagTetra(yc1D, yc1H, yc1BDHF, yc1M, &rD, &rH, &rF, &rB, &rC, &rG, &rE, &rA,v1,I1i);
                EPIdiagTetra(yc1H, yc1F, yc1BDHF, yc1M, &rH, &rF, &rB, &rD, &rG, &rE, &rA, &rC,v1,I1i);
                EPIdiagTetra(yc1F, yc1B, yc1BDHF, yc1M, &rF, &rB, &rD, &rH, &rE, &rA, &rC, &rG,v1,I1i);
                
                EPIdiagTetra(yc1A, yc1B, yc1ABFE, yc1M, &rA, &rB, &rF, &rE, &rC, &rD, &rH, &rG,v1,I1i);
                EPIdiagTetra(yc1B, yc1F, yc1ABFE, yc1M, &rB, &rF, &rE, &rA, &rD, &rH, &rG, &rC,v1,I1i);
                EPIdiagTetra(yc1F, yc1E, yc1ABFE, yc1M, &rF, &rE, &rA, &rB, &rH, &rG, &rC, &rD,v1,I1i);
                EPIdiagTetra(yc1E, yc1A, yc1ABFE, yc1M, &rE, &rA, &rB, &rF, &rG, &rC, &rD, &rH,v1,I1i);
                
                EPIdiagTetra(yc1C, yc1A, yc1CAEG, yc1M, &rC, &rA, &rE, &rG, &rD, &rB, &rF, &rH,v1,I1i);
                EPIdiagTetra(yc1A, yc1E, yc1CAEG, yc1M, &rA, &rE, &rG, &rC, &rB, &rF, &rH, &rD,v1,I1i);
                EPIdiagTetra(yc1E, yc1G, yc1CAEG, yc1M, &rE, &rG, &rC, &rA, &rF, &rH, &rD, &rB,v1,I1i);
                EPIdiagTetra(yc1G, yc1C, yc1CAEG, yc1M, &rG, &rC, &rA, &rE, &rH, &rD, &rB, &rF,v1,I1i);
                
                EPIdiagTetra(yc1D, yc1C, yc1DCGH, yc1M, &rD, &rC, &rG, &rH, &rB, &rA, &rE, &rF,v1,I1i);
                EPIdiagTetra(yc1C, yc1G, yc1DCGH, yc1M, &rC, &rG, &rH, &rD, &rA, &rE, &rF, &rB,v1,I1i);
                EPIdiagTetra(yc1G, yc1H, yc1DCGH, yc1M, &rG, &rH, &rD, &rC, &rE, &rF, &rB, &rA,v1,I1i);
                EPIdiagTetra(yc1H, yc1D, yc1DCGH, yc1M, &rH, &rD, &rC, &rG, &rF, &rB, &rA, &rE,v1,I1i);
                
                EPIdiagTetra(yc1E, yc1F, yc1EFHG, yc1M, &rE, &rF, &rH, &rG, &rA, &rB, &rD, &rC,v1,I1i);
                EPIdiagTetra(yc1F, yc1H, yc1EFHG, yc1M, &rF, &rH, &rG, &rE, &rB, &rD, &rC, &rA,v1,I1i);
                EPIdiagTetra(yc1H, yc1G, yc1EFHG, yc1M, &rH, &rG, &rE, &rF, &rD, &rC, &rA, &rB,v1,I1i);
                EPIdiagTetra(yc1G, yc1E, yc1EFHG, yc1M, &rG, &rE, &rF, &rH, &rC, &rA, &rB, &rD,v1,I1i);
                
                //---------------------------------
                
                EPIdiagTetra(yc2A, yc2B, yc2ABDC, yc2M, &rA, &rB, &rD, &rC, &rE, &rF, &rH, &rG,v2,I2i);
                EPIdiagTetra(yc2B, yc2D, yc2ABDC, yc2M, &rB, &rD, &rC, &rA, &rF, &rH, &rG, &rE,v2,I2i);
                EPIdiagTetra(yc2D, yc2C, yc2ABDC, yc2M, &rD, &rC, &rA, &rB, &rH, &rG, &rE, &rF,v2,I2i);
                EPIdiagTetra(yc2C, yc2A, yc2ABDC, yc2M, &rC, &rA, &rB, &rD, &rG, &rE, &rF, &rH,v2,I2i);
                
                EPIdiagTetra(yc2B, yc2D, yc2BDHF, yc2M, &rB, &rD, &rH, &rF, &rA, &rC, &rG, &rE , v2, -I2i);
                EPIdiagTetra(yc2D, yc2H, yc2BDHF, yc2M, &rD, &rH, &rF, &rB, &rC, &rG, &rE, &rA , v2, -I2i);
                EPIdiagTetra(yc2H, yc2F, yc2BDHF, yc2M, &rH, &rF, &rB, &rD, &rG, &rE, &rA, &rC , v2, -I2i);
                EPIdiagTetra(yc2F, yc2B, yc2BDHF, yc2M, &rF, &rB, &rD, &rH, &rE, &rA, &rC, &rG , v2, -I2i);
                
                EPIdiagTetra(yc2A, yc2B, yc2ABFE, yc2M, &rA, &rB, &rF, &rE, &rC, &rD, &rH, &rG , v2, -I2i);
                EPIdiagTetra(yc2B, yc2F, yc2ABFE, yc2M, &rB, &rF, &rE, &rA, &rD, &rH, &rG, &rC , v2, -I2i);
                EPIdiagTetra(yc2F, yc2E, yc2ABFE, yc2M, &rF, &rE, &rA, &rB, &rH, &rG, &rC, &rD , v2, -I2i);
                EPIdiagTetra(yc2E, yc2A, yc2ABFE, yc2M, &rE, &rA, &rB, &rF, &rG, &rC, &rD, &rH , v2, -I2i);
                
                EPIdiagTetra(yc2C, yc2A, yc2CAEG, yc2M, &rC, &rA, &rE, &rG, &rD, &rB, &rF, &rH , v2, -I2i);
                EPIdiagTetra(yc2A, yc2E, yc2CAEG, yc2M, &rA, &rE, &rG, &rC, &rB, &rF, &rH, &rD , v2, -I2i);
                EPIdiagTetra(yc2E, yc2G, yc2CAEG, yc2M, &rE, &rG, &rC, &rA, &rF, &rH, &rD, &rB , v2, -I2i);
                EPIdiagTetra(yc2G, yc2C, yc2CAEG, yc2M, &rG, &rC, &rA, &rE, &rH, &rD, &rB, &rF , v2, -I2i);
                
                EPIdiagTetra(yc2D, yc2C, yc2DCGH, yc2M, &rD, &rC, &rG, &rH, &rB, &rA, &rE, &rF , v2, -I2i);
                EPIdiagTetra(yc2C, yc2G, yc2DCGH, yc2M, &rC, &rG, &rH, &rD, &rA, &rE, &rF, &rB , v2, -I2i);
                EPIdiagTetra(yc2G, yc2H, yc2DCGH, yc2M, &rG, &rH, &rD, &rC, &rE, &rF, &rB, &rA , v2, -I2i);
                EPIdiagTetra(yc2H, yc2D, yc2DCGH, yc2M, &rH, &rD, &rC, &rG, &rF, &rB, &rA, &rE , v2, -I2i);
                
                EPIdiagTetra(yc2E, yc2F, yc2EFHG, yc2M, &rE, &rF, &rH, &rG, &rA, &rB, &rD, &rC , v2, -I2i);
                EPIdiagTetra(yc2F, yc2H, yc2EFHG, yc2M, &rF, &rH, &rG, &rE, &rB, &rD, &rC, &rA , v2, -I2i);
                EPIdiagTetra(yc2H, yc2G, yc2EFHG, yc2M, &rH, &rG, &rE, &rF, &rD, &rC, &rA, &rB , v2, -I2i);
                EPIdiagTetra(yc2G, yc2E, yc2EFHG, yc2M, &rG, &rE, &rF, &rH, &rC, &rA, &rB, &rD , v2, -I2i);                
                    // write into result vector (private --> shared)
                    // scale vecs
                    // get pointer to A
                    wptr = diag + idn;
